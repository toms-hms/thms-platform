...
async function seed() {
  ...
  const jobData: { category: TradeCategory; title: string; description: string }[] = [
    { category: TradeCategory.PLUMBING,            title: 'Fix kitchen sink leak',          description: 'Slow drain and dripping faucet under the kitchen sink.' },
    { category: TradeCategory.ELECTRICAL,          title: 'Replace outdoor outlets',        description: 'Two outdoor GFCI outlets need replacement; one is not functioning.' },
    { category: TradeCategory.HVAC,                title: 'Annual AC tune-up',              description: 'Pre-summer inspection and tune-up of central air conditioning unit.' },
    { category: TradeCategory.ROOFING,             title: 'Repair storm-damaged shingles',  description: 'Several shingles were blown off during the last storm; need inspection and repair.' },
    { category: TradeCategory.PAINTING,            title: 'Interior living room repaint',   description: 'Paint living room and hallway — approx 400 sq ft, walls only.' },
    { category: TradeCategory.LANDSCAPING,         title: 'Spring yard cleanup',            description: 'Trim hedges, clear flower beds, and aerate the back lawn.' },
    { category: TradeCategory.GENERAL_CONTRACTING, title: 'Kitchen cabinet refresh',        description: 'Replace cabinet doors and hardware in the kitchen.' },
    { category: TradeCategory.CARPENTRY,           title: 'Build backyard deck',            description: 'New 12x16 ft cedar deck off the back door.' },
    { category: TradeCategory.FLOORING,            title: 'Replace master bedroom carpet',  description: 'Remove old carpet and install hardwood flooring in master bedroom (~200 sq ft).' },
    { category: TradeCategory.PEST_CONTROL,        title: 'Ant and termite inspection',     description: 'Noticed ant trails along the foundation; need inspection and treatment.' },
  ];

  for (const j of jobData) {
    await db.insert(jobs).values({
      id: createId(), homeId: home1Id, title: j.title, category: j.category,
