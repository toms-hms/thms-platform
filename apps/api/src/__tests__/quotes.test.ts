...
async function cleanup() {
  await prisma.home.deleteMany({ where: { users: { some: { user: { email: TEST_EMAIL } } } } });
  await prisma.contractor.deleteMany({ where: { users: { some: { user: { email: TEST_EMAIL } } } } });
...
async function setup() {
  ...
  const jobRes = await request(app)
    .post(`/api/v1/homes/${homeRes.body.data.id}/jobs`)
    .set('Authorization', `Bearer ${token}`)
    .send({ title: 'Quote Job', category: 'deck' });

  const ctrRes = await request(app)
    .post('/api/v1/contractors')
    .set('Authorization', `Bearer ${token}`)
    .send({ name: 'Quote Ctr', category: 'deck' });

  // Attach contractor to job
  await request(app)
    .post(`/api/v1/jobs/${jobRes.body.data.id}/contractors`)
    .set('Authorization', `Bearer ${token}`)
    ...
    it('should create a quote', async () => {
      const res = await request(app)
        ...
        .send({
          contractorId,
          amount: 12000,
          description: 'Cedar deck estimate',
          status: 'DRAFT',
        });
  ...
  describe('DELETE /api/v1/quotes/:quoteId', () => {
    it('should delete a quote', async () => {
      ...
      const delRes = await request(app)
        .delete(`/api/v1/quotes/${createRes.body.data.id}`)
        .set('Authorization', `Bearer ${token}`);
